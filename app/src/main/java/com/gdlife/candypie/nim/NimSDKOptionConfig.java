...
//        initStatusBarNotificationConfig(options);
        ...
        // 通知栏显示用户昵称和头像
        options.userInfoProvider = new NimUserInfoProvider(MAPP.mapp);
...
////
        ...
        if (UserService.getInstance().getUser() != null && UserService.getInstance().getUser().getSound() == 1) {
            config.ring = true;
        } else if (UserService.getInstance().getUser() != null && UserService.getInstance().getUser().getSound() == 0) {
            config.ring = false;
        }

        if (UserService.getInstance().getUser() != null && UserService.getInstance().getUser().getDisplay_content() == 1) {
            config.titleOnlyShowAppName = false;
        } else if (UserService.getInstance().getUser() != null && UserService.getInstance().getUser().getDisplay_content() == 0) {
            config.titleOnlyShowAppName = true;
        }

        // 通知铃声的uri字符串
        config.notificationSound = "android.resource://com.gdlife.candypie/raw/new_message";
...
//        configServerAddress(options);
    ...
    public static String getAppCacheDir(Context context) {
        ...
        if (TextUtils.isEmpty(storageRootPath)) {
            ...
            storageRootPath = Environment.getExternalStorageDirectory() + "/" + MAPP.mapp.getPackageName();
        }
...
//    }
    ...
    private static StatusBarNotificationConfig loadStatusBarNotificationConfig() {
        StatusBarNotificationConfig config = new StatusBarNotificationConfig();
        // 点击通知需要跳转到的界面
        config.notificationEntrance = IndexActivity.class;
        config.notificationSmallIconId = R.drawable.ic_launcher_background;
        config.notificationColor = MAPP.mapp.getResources().getColor(R.color.theme_color);
        // 通知铃声的uri字符串
...
//        DemoCache.setNotificationConfig(config);
    ...
    private static MixPushConfig buildMixPushConfig() {
        ...
        // 小米推送
        config.xmAppId = "2882303761517757484";
        config.xmAppKey = "5911775716484";
        ...
        // 魅族推送
//        config.mzAppId = "1000031";
//        config.mzAppKey = "29fc2e1946014c8d99eafe44fc89db7b";
