...
import com.heboot.utils.DisplayUtils;
...
public class LuckPanLayout extends RelativeLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        MinValue -= QMUIDisplayHelper.dp2px(context,10)*2;
//        Logger.getLogger().d("screenWidth = "+screenWidth + "screenHeight = "+screeHeight + "MinValue = "+MinValue);
    ...
    private void drawSmallCircle(boolean FirstYellow){
        int pointDistance = radius - DisplayUtils.dip2px(context,10);
        for(int i=0;i<=360;i+=20){
            int x = (int) (pointDistance * Math.sin(DisplayUtils.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(DisplayUtils.change(i))) + CircleY;

            if(FirstYellow)
                canvas.drawCircle(x,y,DisplayUtils.dip2px(context,4),yellowPaint);
            else
                canvas.drawCircle(x,y,DisplayUtils.dip2px(context,4),whitePaint);
