...
public class HomepageOtherActivity2 extends BaseActivity<FragmentHomepageBottomBinding> {
    ...
    public void initListener() {
        ...
        binding.layoutHomepageContent.includeUserinfo.ivEdit.setOnClickListener((v) -> {
            IntentUtils.toUserInfoActivity(HomepageOtherActivity2.this, MValue.FROM_MY, MValue.USER_INFO_TYPE_EDIT, user, null, null);
        });
        ...
        binding.tvLeft.setOnClickListener((v) -> {
            ...
            if (user.getId().intValue() == UserService.getInstance().getUser().getId().intValue()) {
                tipDialog = DialogUtils.getFailDialog(HomepageOtherActivity2.this, "不能对自己操作", true);
                tipDialog.show();
                return;
            }
            postVideoService();
        ...
        binding.ivToyue.setOnClickListener((v) -> {
            ...
            IntentUtils.toThemeListActivity(HomepageOtherActivity2.this, true, user);
        });

        binding.ivTochat.setOnClickListener((v) -> {
            if (UserService.getInstance().checkTourist(HomepageOtherActivity2.this)) {
                return;
            }
            if (user.getId().intValue() == UserService.getInstance().getUser().getId().intValue()) {
                tipDialog = DialogUtils.getFailDialog(HomepageOtherActivity2.this, "不能对自己操作", true);
            ...
            NimChatService.getInstance().toChat(HomepageOtherActivity2.this, user);
        });
        ...
        binding.vMore.setOnClickListener((v) -> {
            ...
            bottomSheetDialog = DialogUtils.getHomepageBottomSheet(HomepageOtherActivity2.this, new Consumer<Integer>() {
                @Override
                public void accept(Integer integer) throws Exception {
                    if (integer == 1) {
                        ...
                        if (user.getIs_black() == 1) {
                            HttpClient.Builder.getGuodongServer().un_black(params).observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.io()).subscribe(new HttpObserver<BaseBeanEntity>() {
                                ...
                                public void onSuccess(BaseBean<BaseBeanEntity> baseBean) {
                                    tipDialog = DialogUtils.getSuclDialog(HomepageOtherActivity2.this, baseBean.getMessage(), true);
                                    tipDialog.show();
                                }

                                @Override
                                public void onError(BaseBean<BaseBeanEntity> baseBean) {
                                    tipDialog = DialogUtils.getFailDialog(HomepageOtherActivity2.this, baseBean.getMessage(), true);
                                    tipDialog.show();
                                }
                            });
                        } else {
                        ...
                        bottomSheetDialog.dismiss();
                        IntentUtils.toReportActivity(HomepageOtherActivity2.this, String.valueOf(user.getId()), ReportFromType.REPROT);
                    }
                }
            }, user.getIs_black() == 1);
    ...
    private void postVideoService() {
        videoChatService.postVideoService(permissionUtils, (BaseActivity) HomepageOtherActivity2.this, user, coinDialog);
    }
...
//        binding.layoutHomepageContent.includeVer.includeIdVer.tvStatus.setText(getString(R.string.ver_ok));
        ...
        if (user.getSkill_list() != null && user.getSkill_list().size() > 0) {
            binding.layoutHomepageContent.includeSkill.rvList.setLayoutManager(new GridLayoutManager(HomepageOtherActivity2.this, 3) {
                @Override
        ...
        if (user.getVideo_list() != null && user.getVideo_list().size() > 0) {
            ...
            binding.layoutHomepageContent.includeVideos.rvList.setLayoutManager(new LinearLayoutManager(HomepageOtherActivity2.this, LinearLayoutManager.HORIZONTAL, false) {
                @Override
                public boolean canScrollVertically() {
                    return false;
                }
...
//        }
    ...
    private void initVisitUsers() {
        if (user.getVisit_list() != null && user.getVisit_list().getList() != null && user.getVisit_list().getList().size() > 0) {
            ...
            binding.layoutHomepageContent.includeIndexVisit.rvList.setLayoutManager(new LinearLayoutManager(HomepageOtherActivity2.this, LinearLayoutManager.HORIZONTAL, false) {
                @Override
                public boolean canScrollHorizontally() {
                    return false;
                }
            ...
            binding.layoutHomepageContent.includeIndexVisit.getRoot().setOnClickListener((v) -> {
                IntentUtils.toUserVisitActivity(HomepageOtherActivity2.this, String.valueOf(UserService.getInstance().getUser().getId()));
            });
        } else {
