...
public class HomepageSelectKGDialog extends Dialog {

    public HomepageSelectKGDialog(Context context) {
        super(context);
    }

    public HomepageSelectKGDialog(Context context, int theme) {
        super(context, theme);
    }
    ...
    public static class Builder {
        ...
        public HomepageSelectKGDialog create() {

            List<String> reawrdDatas = HomepageService.getChooseKGs();

            int selectIndex = 0;

            for (int i = 0; i < reawrdDatas.size(); i++) {
                if (reawrdDatas.get(i).equals("45")) {
                    selectIndex = i;
                    continue;
                }
            }

            final HomepageSelectKGDialog dialog = new HomepageSelectKGDialog(context, R.style.zhezhao_dialog);
            ...
            binding.includeTop.tvTitme.setText(MAPP.mapp.getString(R.string.choose_kg));

            binding.tvUnit.setText(MAPP.mapp.getString(R.string.unit_kg));
