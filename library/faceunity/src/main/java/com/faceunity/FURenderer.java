...
public class FURenderer implements OnFUControlListener {
    ...
    public byte[] onDrawFrame2(byte[] img, int tex, int w, int h) {
        if (tex <= 0 || img == null || w <= 0 || h <= 0) {
            Log.e(TAG, "onDrawFrame date null");
            return null;
        }
     ...
     * @param img
     */
    public void drawFrame(int tex, int w, int h, int cameraFacing, byte[] img) {
        if (cameraFacing != mCurrentCameraType) {
            onCameraChange(cameraFacing, 0);
        }
        prepareDrawFrame();
        faceunity.fuRenderToTexture(tex, w, h, mFrameId++, mItemsArray, faceunity.FU_ADM_FLAG_EXTERNAL_OES_TEXTURE, img, w, h);
    }
